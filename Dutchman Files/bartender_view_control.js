...
function createElement(productID, quantity) {
    ...
    elem.onclick = function () { quantity = change_order(name, quantity) };
    ...
    for (i = 0; i < DB_order_example.length; i++) {
        if (DB_order_example[i].tableNr == tableNr && DB_order_example[i].paid === false) {
            ...
            for (j in tmp) {
                ...
                var list_tmp = tmp[j];
                for (k in list_tmp) {
                    var productID = list_tmp[k][0];
                    var quantity = list_tmp[k][1];
                    //console.log(iid,q);
            ...
            console.log(DB_order_example[i]);
            DB_order_example[i]["suborder"] = tmp;//the js seem don't know what it is,very strange
            break;
        }
    }
...
function change_order(name, quantity) {
    toggleWindow("change_order"); name + " : " + quantity
    $('#order_form').html(name + " : " + quantity + "\r");
    var btn = document.getElementById('add_order');
    btn.addEventListener('click', function () {
        quantity = quantity + 1;
        $('#order_form').html(name + " : " + quantity + "\r");
        $('#' + name).html(name + " : " + quantity);
    });
...
function add_refill(ID, price) {
    ...
    if (typeof refillOrder[ID] != "undefined") {
        refillOrder[ID][1] += 1;
    }//if this element exists in the order, we increase the count 

    else {
        refillOrder[ID] = [Number(price), 1];
    } // Else, create element in order
...
function remove_refill(ID, price) {
    ...
    if (refillOrder[ID][1] > 1) {
        refillOrder[ID][1] -= 1;
    }//if this element exists in the order, we decrease the count 

    else {
        delete refillOrder[ID];
    } // Else, delete element in order
    total_price_refill -= Number(price)
    updateRefillOrder();
}
